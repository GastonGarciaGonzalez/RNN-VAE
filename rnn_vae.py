...
from tensorflow.keras.layers import Layer, Input, SimpleRNN, LSTM, GRU, BatchNormalization, Lambda
...
class RNNVAE:
    ...
    def __init__(self,
        ...
        # First
        if layer_type == 'rnn':
            h_enc_rnn = SimpleRNN(rnn_units[0], activation='tanh',
                            return_sequences=True, name='rnn_%d'%0)(inputs)
        elif layer_type == 'gru':
            h_enc_rnn = GRU(rnn_units[0], activation='tanh',
                            return_sequences=True, name='gru_%d'%0)(inputs)
        elif layer_type == 'lstm':
            h_enc_rnn = LSTM(rnn_units[0], activation='tanh',
                            return_sequences=True, name='lstm_%d'%0)(inputs)
        h_enc_rnn = BatchNormalization()(h_enc_rnn)
        
        # Middle 
        for i in range(len(rnn_units)-2):
            if layer_type == 'rnn':
                h_enc_rnn = SimpleRNN(rnn_units[i+1], activation='tanh',
                            return_sequences=True, name='rnn_%d'%(i+1))(h_enc_rnn)
            elif layer_type == 'gru':
                h_enc_rnn = GRU(rnn_units[i+1], activation='tanh',
                            return_sequences=True, name='gru_%d'%(i+1))(h_enc_rnn)
            elif layer_type == 'lstm':
                h_enc_rnn = LSTM(rnn_units[i+1], activation='tanh',
                            return_sequences=True, name='lstm_%d'%(i+1))(h_enc_rnn)
            h_enc_rnn = BatchNormalization()(h_enc_rnn)
            
        # Lastest    
        if layer_type == 'rnn':
            z_mean = SimpleRNN(self.J, activation=None,
                        return_sequences=True, name='z_mean')(h_enc_rnn)
        elif layer_type == 'gru':
            z_mean = GRU(self.J, activation=None,
                        return_sequences=True, name='z_mean')(h_enc_rnn)
        elif layer_type == 'lstm':
            z_mean = LSTM(self.J, activation=None,
                        return_sequences=True, name='z_mean')(h_enc_rnn)

        if layer_type == 'rnn':
            z_log_var = SimpleRNN(self.J, activation=None,
                        return_sequences=True, name='z_log_var')(h_enc_rnn)
        elif layer_type == 'gru':
            z_log_var = GRU(self.J, activation=None,
                        return_sequences=True, name='z_log_var')(h_enc_rnn)
        elif layer_type == 'lstm':
            z_log_var = LSTM(self.J, activation=None,
                        return_sequences=True, name='z_log_var')(h_enc_rnn)
        ...
        # First
        if layer_type == 'rnn':
            h_dec_rnn = SimpleRNN(rnn_units[-1], activation='tanh',
                            return_sequences=True, name='rnn_-1')(latent_inputs)
        elif layer_type == 'gru':
            h_dec_rnn = GRU(rnn_units[-1], activation='tanh',
                            return_sequences=True, name='gru_-1')(latent_inputs)
        elif layer_type == 'lstm':
            h_dec_rnn = LSTM(rnn_units[-1], activation='tanh',
                            return_sequences=True, name='lstm_-1')(latent_inputs)
        h_dec_rnn = BatchNormalization()(h_dec_rnn)       
        
        #Middle
        for i in range(-2, -len(rnn_units)-1, -1):
            if layer_type == 'rnn':
                h_dec_rnn = SimpleRNN(rnn_units[i], activation='tanh',
                            return_sequences=True, name='rnn_%d'%i)(h_dec_rnn)
            elif layer_type == 'gru':
                h_dec_rnn = GRU(rnn_units[i], activation='tanh',
                            return_sequences=True, name='gru_%d'%i)(h_dec_rnn)
            elif layer_type == 'lstm':
                h_dec_rnn = LSTM(rnn_units[i], activation='tanh',
                            return_sequences=True, name='lstm_%d'%i)(h_dec_rnn)
            h_dec_rnn = BatchNormalization()(h_dec_rnn)
            
        # Lastest/Output
        if layer_type == 'rnn':
            x__mean = SimpleRNN(self.M, activation=None,
                        return_sequences=True, name='x__mean_output')(h_dec_rnn)
        elif layer_type == 'gru':
            x__mean = GRU(self.M, activation=None,
                        return_sequences=True, name='x__mean_output')(h_dec_rnn)
        elif layer_type == 'lstm':
            x__mean = LSTM(self.M, activation=None,
                        return_sequences=True, name='x__mean_output')(h_dec_rnn)
        

        if layer_type == 'rnn':
            x_log_var = SimpleRNN(self.M, activation=None,
                        return_sequences=True, name='x_log_var_output')(h_dec_rnn)
        elif layer_type == 'gru':
            x_log_var = GRU(self.M, activation=None,
                        return_sequences=True, name='x_log_var_output')(h_dec_rnn)
        elif layer_type == 'lstm':
            x_log_var = LSTM(self.M, activation=None,
                        return_sequences=True, name='x_log_var_output')(h_dec_rnn)
